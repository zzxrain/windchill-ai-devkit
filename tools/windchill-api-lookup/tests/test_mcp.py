...
def test_mcp_tool_contracts_and_readonly_queries_do_not_mutate(
    ...
    async def check():
        async with Client(
            ...
            ensure_tool = tools_by_name[
                "ensure_javadoc_index"
            ]

            assert (
                ensure_tool.annotations.read_only_hint
                is False
            )

            assert (
                ensure_tool.annotations.destructive_hint
                is False
            )

            assert (
                ensure_tool.annotations.idempotent_hint
                is True
            )

            assert (
                ensure_tool.annotations.open_world_hint
                is False
            )

            assert (
                "version"
                in ensure_tool.input_schema["properties"]
            )
            ...
            assert (
                "home"
                not in ensure_tool.input_schema["properties"]
            )
            ...
            for name, tool in tools_by_name.items():
                ...
                assert (
                    "home"
                    not in tool.input_schema["properties"]
                )
...
def test_ensure_javadoc_index_builds_and_reuses_future_version(
    tmp_path,
):
    ...
    async def check():
        async with Client(
            ...
            assert (
                first.structured_content["ok"]
                is True
            )

            assert (
                first.structured_content["reused"]
                is False
            )
            ...
            assert (
                query.structured_content["version"]
                == FUTURE_VERSION
            )
            ...
            assert (
                second.structured_content["reused"]
                is True
            )
...
def test_ensure_javadoc_index_requires_absolute_path(
    tmp_path,
):
    home = (
        tmp_path
...
def test_corrupt_index_preserves_error_code(
    indexed_home,
):
    (
        indexed_home
